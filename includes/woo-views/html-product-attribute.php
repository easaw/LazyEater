<div data-taxonomy="<?php echo esc_attr( $taxonomy ); ?>" class="woocommerce_attribute wc-metabox closed <?php echo esc_attr( implode( ' ', $metabox_class ) ); ?>" rel="<?php echo $position; ?>">
	<h3>
		<a href="#" class="remove_row delete"><?php _e( 'Remove', 'frozr' ); ?></a>
		...
		<table cellpadding="0" cellspacing="0">
			<tbody>
				<tr>
					...
					<td rowspan="3">
						<label><?php _e( 'Value(s)', 'frozr' ); ?>:</label>

						<?php if ( $attribute['is_taxonomy'] ) : ?>
							<?php if ( 'select' === $attribute_taxonomy->attribute_type ) : ?>

								<select multiple="multiple" data-placeholder="<?php esc_attr_e( 'Select terms', 'frozr' ); ?>" class="multiselect attribute_values wc-enhanced-select" name="attribute_values[<?php echo $i; ?>][]">
									<?php
								...
								<button class="button plus select_all_attributes"><?php _e( 'Select all', 'frozr' ); ?></button>
								<button class="button minus select_no_attributes"><?php _e( 'Select none', 'frozr' ); ?></button>
								<button class="button fr plus add_new_attribute"><?php _e( 'Add new', 'frozr' ); ?></button>
							...
							<?php elseif ( 'text' == $attribute_taxonomy->attribute_type ) : ?>
								...
								?>" placeholder="<?php echo esc_attr( sprintf( __( '"%s" separate terms', 'frozr' ), WC_DELIMITER ) ); ?>" />
						...
						<?php else : ?>
							...
							<textarea name="attribute_values[<?php echo $i; ?>]" cols="5" rows="5" placeholder="<?php echo esc_attr( sprintf( __( 'Enter some text, or some attributes by "%s" separating values.', 'frozr' ), WC_DELIMITER ) ); ?>"><?php echo esc_textarea( $attribute['value'] ); ?></textarea>
				...
				<tr>
					<td>
						<label><input type="checkbox" class="checkbox" <?php checked( $attribute['is_visible'], 1 ); ?> name="attribute_visibility[<?php echo $i; ?>]" value="1" /> <?php _e( 'Visible on the product page', 'frozr' ); ?></label>
					</td>
				</tr>
				<tr>
					<td>
						<div class="enable_variation show_if_variable">
							<label><input type="checkbox" class="checkbox" <?php checked( $attribute['is_variation'], 1 ); ?> name="attribute_variation[<?php echo $i; ?>]" value="1" /> <?php _e( 'Used for variations', 'frozr' ); ?></label>
						</div>
					</td>
				</tr>
			</tbody>
